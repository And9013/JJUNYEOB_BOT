...
const command = {
    노래: async (msg, args) => {
        ...
        if(!voiceChannel) return msg.channel.send(fmt("음성채널에 먼저 들어가세요."));
    ...
    일시정지: async (msg, args) => {
        ...
        msg.channel.send(fmt("노래 일시 정지함"));
    },
    일시정지해제: async (msg, args) => {
        const voiceChannel = msg.member.voice.channel;
        if(!voiceChannel) return msg.channel.send(fmt("Connect to a voice channel first!"));
        ...
        msg.channel.send(fmt("일시정지 해제"));
    },
    대기열추가: async (msg, args) => {
        if(!queue.has(msg.guild.id)) return msg.channel.send(fmt("There is no queue playing!"));
    ...
    대기열에서삭제: async (msg, args) => {
        ...
        msg.channel.send(fmt("대기열에서 삭제함"));
    },
    연속재생: async (msg, args) => {
        const voiceChannel = msg.member.voice.channel;
        if(!voiceChannel) return msg.channel.send(fmt("Connect to a voice channel first!"));
